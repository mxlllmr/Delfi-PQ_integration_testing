...
The main goal is to verify that the current consumption of the EPS bus line 3 reafding with its sensors is correct. For that, a digital potentiometer is used, which acts like an analog potentiometer (variable resistor), but the resistance value is changed via digital inputs [[4]](https://www.analogictips.com/what-is-a-digital-potentiometer/). The digital potentiometer is connected to the Arduino, which controls the potentiometer's resitance value, influencing the current consumption to the connected bus. Through that, the current in the EPS bus line 3 can be changed, which is captured by the EPS' internal sensors. The sensor value can be observed by the user with the respective python scripts, which receive the current value of bus 3 through the houskeeping request sent to the EPS. Following, if the observed current value coincides with the set resistance value, the EPS sensor readings are considered to be correct.
